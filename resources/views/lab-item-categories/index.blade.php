<x-admin-layout>
    <div class="xl:container px-3 py-4">
        <div class="flex mb-3 justify-between">
            <div class="">
                <h1 class="text-2xl font-semibold">Lab Item Categories</h1>
            </div>
            <div class="self-center">
                <form action="{{ route('lab-item-categories.store') }}" method="post" data-js="form-model-create">
            ...
            <table class="w-full bg-white">
                <thead>
                    <tr class="border-b">
                        ...
                        <th scope="col" class="px-3 py-2 text-start">Lab Items</th>
                    ...
                    @if(!empty($items) && count($items))
                        @foreach($items as $item)
                            <tr class="border-b">
                                ...
                                <td class="px-3 py-2">{{ $item->lab_items_count }}</td>
                                <td class="px-3 py-2">
                                    <a href="{{ route('lab-item-categories.edit', $item) }}">Edit</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
